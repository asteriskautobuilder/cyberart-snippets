...
(function(){
  ...
  g.musicTick=saneNum(g.musicTick,0);g.musicStep=saneNum(g.musicStep,0);g.musicBar=saneNum(g.musicBar,0);g.musicNotes=saneNum(g.musicNotes,0);g.musicLayer=saneNum(g.musicLayer,1);
  ...
  function music(){if(g.screen!=='play')return;if(g.audioArmed&&!g.audioReady)armAudio();if(!g.audioReady)return;var ac=armAudio();if(!audioLive(ac))return;g.musicTick--;if(g.musicTick>0)return;g.musicTick=9;var layer=Math.max(1,Math.min(5,g.level||1));g.musicLayer=layer;var lead=[220,0,247,294,0,247,196,0,165,196,220,0,247,196,165,0],bass=[55,0,55,0,49,0,49,0,65,0,62,0,55,0,73,0],harmony=[0,330,0,392,349,0,330,0,294,0,330,392,0,349,294,0],arp=[440,554,659,740,659,554,494,392,330,392,494,554,494,392,330,277],choir=[110,0,165,0,146,0,196,0,220,0,196,0,165,0,146,0],step=g.musicStep++%16;g.musicBar=Math.floor(g.musicStep/16);note(ac,lead[step],.16,'triangle',.017,0);if(step%2===0)note(ac,bass[step],.2,'square',.017,0);if(layer>=2&&step%4===0)note(ac,82,.22,'sine',.02,0);if(layer>=2&&step%4===2)note(ac,98,.16,'sine',.014,0);if(layer>=3)note(ac,harmony[step],.12,'triangle',.012,.03);if(layer>=3&&step%2===1)note(ac,arp[(step+g.level)%16],.055,'sine',.008,.06);if(layer>=4&&(step%4===1||step%4===3))note(ac,660,.026,'square',.006,0);if(layer>=4&&step%8===4)note(ac,124,.045,'sawtooth',.012,0);if(layer>=5){note(ac,choir[step],.24,'sine',.014,.02);if(step%8===7)note(ac,880,.08,'triangle',.01,.09);}}
  ...
  function dropAt(x,y,e){
    ...
    var tier=e.boss?3:(e.xp>=44?2:e.xp>=26?1:0),cw=[16,18,21,24][tier],ch=[18,21,24,28][tier],xpSurf=surfaceForDrop(x+10,cw,y-10,e),variant=(Math.floor((x+y+(g.level||1)*7))%3+3)%3;drops.push({x:xpSurf.x,y:xpSurf.y-ch,w:cw,h:ch,kind:'xp',label:'xp',color:'#55ffa9',xp:e.xp,life:900,tier:tier,variant:variant,spark:(g.frame||0)%16});
  }
...
(function(){
  ...
  function drawCrystal(d){var w=d.w||18,h=d.h||18,tier=d.tier||0,v=d.variant||0,pulse=Math.floor((Math.sin(((g.frame||0)+v*9)/9)+1)*2),cx=w/2;px(cx-3,0,6,4,'#d8ffe1');px(cx-7,4,14,5,'#7dff9e');px(cx-10,9,w+2,Math.max(5,h-14),'#19b96e');px(cx-5,6,8,h-7,'#55ffa9');px(cx+3,8,5,h-12,'#0a754f');px(cx-12,h-8,6,6,'#0e5b42');px(cx+7,h-9,6,7,'#8effc3');if(tier>0){px(cx-14,h-14,6,10,'#2bdd83');px(cx+10,h-16,7,12,'#129c65');}if(tier>1){px(cx-4,h-4,8,5,'#e8ffe6');px(cx-1,2,2,h-8,'#b8ffd0');}if(pulse>1){px(cx-13-v,h-18+v,3,3,'#eafff0');px(cx+9+v,3+v,2,2,'#eafff0');}}
  function drawPotion(d){var w=d.w||24,h=d.h||24;px(8,0,8,4,'#6b4a2b');px(9,4,6,4,'#d9f7ff');px(6,8,12,4,'#bdefff');px(4,12,16,9,'#ff5577');px(6,15,12,7,'#c51f54');px(7,10,4,9,'#ffdce3');px(16,14,2,5,'#7a0d32');px(3,20,18,3,'#5b1730');if((g.frame||0)%32<16)px(11,-3,2,2,'#fff4b0');}
  function drawWeaponDrop(d){var glow=Math.floor((g.frame||0)/10)%2;px(3,15,18,5,'#7f4f2a');px(14,5,5,18,'#d7dde8');px(17,4,7,8,'#f1f5ff');px(9,12,12,4,'#f0d36b');px(2,18,5,5,'#3a2419');if(glow)px(21,2,3,3,'#8ef7ff');}
  function drawUpgradeDrop(d){px(5,5,15,15,'#312041');px(8,2,9,5,'#8ef7ff');px(8,18,9,5,'#8ef7ff');px(2,8,5,9,'#8ef7ff');px(18,8,5,9,'#8ef7ff');px(10,7,5,11,'#f0d36b');px(7,10,11,5,'#f0d36b');px(12,12,2,2,'#fff');}
  function drawPowerDrop(d){if(d.power==='heart')return drawPotion(d);var col=d.color||'#fff',p=d.power||'';px(5,6,14,14,'rgba(5,8,18,.78)');px(7,4,10,4,col);px(7,20,10,3,col);px(3,9,4,8,col);px(19,9,4,8,col);if(p==='shield'){px(8,7,8,10,'#9ee8ff');px(10,9,4,8,'#e8fbff');}else if(p==='haste'||p==='spring'){px(7,14,11,3,'#fff06a');px(12,8,5,5,col);px(5,17,5,4,col);}else if(p==='bomb'){px(8,9,10,10,'#24202a');px(12,6,8,4,'#ffce48');px(17,4,3,3,'#fff06a');}else if(p==='clover'||p==='regen'){px(9,8,4,4,'#55ffa9');px(14,8,4,4,'#55ffa9');px(9,13,4,4,'#55ffa9');px(14,13,4,4,'#55ffa9');}else if(p==='tome'||p==='focus'){px(7,7,12,12,'#f5e1a1');px(9,9,8,2,'#3b2b22');px(9,13,7,2,'#3b2b22');}else{px(9,7,7,12,col);px(7,10,11,6,'#fff');}}
  function drawDrop(d){c.save();c.translate(d.x,d.y+Math.sin(((g.frame||0)+(d.variant||0)*8)/18)*2);if(d.kind==='xp')drawCrystal(d);else if(d.kind==='weapon')drawWeaponDrop(d);else if(d.kind==='upgrade')drawUpgradeDrop(d);else drawPowerDrop(d);c.restore();}
  ...
  function background(){
    ...
    for(var p=0;p<plats.length;p++){var pl=plats[p],face=pl.large?'rgba(62,52,67,.98)':'rgba(52,42,59,.95)';px(pl.x,pl.y,pl.w,pl.h,face);px(pl.x,pl.y,pl.w,5,'rgba(139,255,224,.34)');for(var bs=0;bs<pl.w;bs+=28){px(pl.x+bs,pl.y+9,18,2,'rgba(205,192,177,.15)');if(pl.large)px(pl.x+bs+3,pl.y+17,12,4,'rgba(8,6,14,.26)');if((bs+p*7)%56===0)px(pl.x+bs+8,pl.y+3,5,3,'rgba(125,255,158,.24)');if((bs+p*11)%84===0)px(pl.x+bs+18,pl.y+pl.h-5,9,2,'rgba(240,211,107,.18)');}}
  ...
  function drawEnemy(e){
    ...
    if(e.type==='boss'){px(8,12,80,92,'#37112b');px(18,2,16,22,'#ff4f9c');px(40,0,16,26,'#ff4f9c');px(62,2,16,22,'#ff4f9c');px(22,44,16,16,'#090711');px(58,44,16,16,'#090711');px(22,88,52,10,'#f6d06c');px(46,-20,6,38,'#f0d36b');px(30,49,5,5,'#f0d36b');px(66,49,5,5,'#f0d36b');px(36,70,24,5,'#120714');px(14,24,8,5,'#8ef7ff');px(72,24,8,5,'#8ef7ff');}
    else if(e.type==='rat'){px(4,10,26,12,'#6f4f45');px(12,4,18,10,'#8b6b5c');px(28,8,8,8,'#8b6b5c');px(31,7,3,3,'#f0d36b');px(0,18,10,4,'#c49a7a');px(8,22+step*2,6,4,'#3b2722');px(24,22-step*2,6,4,'#3b2722');px(13,9,5,2,'#c8a083');px(25,13,3,2,'#241713');}
    ...
    else if(e.type==='slime'){px(4,12,30,14,'#55cc77');px(8,6,22,8,'#77e095');px(12,8,8,4,'#d2ffcd');px(25,15,4,4,'#102216');px(8,18,5,3,'#168044');px(19,22,9,2,'#2aa75f');}
    else if(e.type==='wisp'){px(7,4,18,26,'rgba(121,221,255,.42)');px(11,0,10,34,'#79ddff');px(13,13,3,3,'#fff');px(20,13,3,3,'#fff');}
    else if(e.type==='skel'){px(11,2,14,14,'#d9d2bd');px(15,16,8,20,'#d9d2bd');px(5,25,8,5,'#d9d2bd');px(24,25,8,5,'#d9d2bd');px(9,38+step*2,7,8,'#d9d2bd');px(22,38-step*2,7,8,'#d9d2bd');px(14,7,3,3,'#101018');px(22,7,3,3,'#101018');px(16,13,7,2,'#8d8370');px(14,23,10,3,'#f0d36b');}
  ...
  function drawPlayer(){
    ...
    px(6,18,22,34,name==='Warrior'?'#5f1f2f':name==='Mage'?'#4c3a92':name==='Archer'?'#183823':'#5d4c22');px(9,22,16,28,cls.color);px(8,28,18,3,'#101018');px(10,32,4,3,'rgba(255,255,255,.38)');
    ...
    px(8,0,18,16,'#f2c994');px(11,7,4,3,'#101018');px(22,7,4,3,'#101018');px(13,13,8,2,'#9b6048');
    px(8,52,7,12+leg*.25,'#1b1028');px(22,52,7,12-leg*.25,'#1b1028');px(6,62+leg*.25,10,4,'#090711');px(21,62-leg*.25,10,4,'#090711');if(g.onRope){px(2,24,7,19,'#f2c994');px(25,24,7,19,'#f2c994');px(15,14,4,45,'rgba(184,135,79,.72)');}else{px(4,30,7,16,'#f2c994');px(25,30,7,16,'#f2c994');}
...
writeJson("update.json", {
  ...
  description: "Class combat, profile-driven longer vertical floors, climbable ropes, dash, pits, XP/stat leveling, boss, and level-layered NES synth audio",
  phase: "update",
...
writeJson("render.json", {
  ...
  description: "Render scaled dungeon roguelike platformer with detailed pixel sprites, crystal/potion pickups, ropes, pits, stable HUD, and menus",
  phase: "render",
  code: renderCode.trim()
});

writeJson("cart.json", {
  id: "glade-dungeon-relic-rogue-v18",
  name: "Dungeon Relic Rogue V18",
  description: "An art and audio polish pass for the NES-flavored roguelike platformer: potion health pickups, glimmering green XP crystals with size variation, more detailed pixel sprites and world surfaces, and level-layered dungeon synth music.",
  frameRate: 60,
  modules: [
    { moduleId: "glade-dungeon-rogue-init", version: 8 },
    { moduleId: "glade-dungeon-rogue-update", version: 18 },
    { moduleId: "glade-dungeon-rogue-render", version: 14 }
  ]
});
